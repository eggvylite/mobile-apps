...
export default function SelectSubscriptionPaymentMethod({ route }) {


  const navigation = useNavigation();
  const dispatch = useDispatch();
  const { provider, plan } = route?.params || {};
      ...
      api.post('advances/' + storedata.id, payload)
        .then(function (response) {
          ...
          navigation.navigate('AdvanceSuccess', {
            type: 'advance',
            amount: activeSub?.plan_cash_upto,
  ...
  const purchasePlan = async () => {
    ...
    try {
      const payload = {
        amount: (plan?.partmonthly ?? 0).toString(),
        customer_id: storedata?.id,
        pm: selectedMethod,
        plan_id: plan?.id,
        type: "monthly",
      ...
      const response = await api.post('customer/paymentcapture', payload);

      const res = await api.get("customer/" + storedata.id);

      if (res?.data?.subscription === 'Yes') {
        const infodata = { ...storedata, plan: res.data.subscription };
        await CommonFunction.storeData('@cusLoginInfo', infodata);
        dispatch(updateAuthdata(infodata));
        dispatch(fetchCustomer());
        dispatch(fetchAuth())

        const findcard = paymentMethods?.find((item) => item?.pm_id === selectedMethod)
        navigation?.replace('SubscriptonSuccessScreen', {
          amount: plan?.partmonthly,
          last4: findcard?.number || findcard?.number || '0000',
          brand: findcard?.name || findcard?.name || 'Card',
          message: response?.data?.Message || 'Your subscription is now active.',
        })
      ...
      appLog?.error?.('purchasePlan failed', err?.response?.data || err?.message);
      CommonFunction.message(err?.response?.data?.Message || 'Purchase failed', 'danger');
      dispatch(fetchNotication(10));
      dispatch(resetTransaction());
    } finally {
      setIsSubmitting(false);
      ...
      >
        ...
        <View style={styles.paymentSummary}>
          <View style={styles.summaryRow}>
            ...
            <Text style={styles.summaryValue}>{plan?.title || 'Not selected'}</Text>
          </View>
          <View style={styles.summaryRow}>
            <Text style={styles.summaryLabel}>Amount</Text>
            <Text style={styles.summaryValue}>
              {storedata?.currency}{CommonFunction.formatamount(plan?.partmonthly)}
            </Text>
          </View>
          {/* <View style={styles.summaryRow}>
