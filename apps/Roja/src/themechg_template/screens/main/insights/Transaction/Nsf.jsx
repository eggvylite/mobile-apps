...
function Nsf(props) {
    ...
    useEffect(() => {
        if (0 < statements.length && currentMonth) {
            ...
            const ch = statements.filter(item => {
                ...
                const matchword = nsfkeyword.length > 0 && new RegExp(`\\b(${nsfkeyword.join("|")})\\b`, "i").test(item.description)
    ...
    const splitWeeklychart = (transactions, month, year, lcolor, rcolor) => {
        ...
        const resultbar = CommonFunction.getweekBardata(weeks, debits,themeColors)
        return resultbar;
    }
                                                ...
                                                dateRange ?
                                                    <View style={{ alignItems: 'center', justifyContent: 'center', margin: 20 }}>
                                                        <Text style={styles.daterangeincreaseDecrease}>{
                                                            timeLine === '1' ? 'This Year' : timeLine === '13' ? 'Last 3 Months' : timeLine === '14' ? 'Last 6 Months' : timeLine === '4' ? 'Last Month' :
                                                                timeLine === '3' ? 'This Month' : timeLine === '7' ? changFormat(fDate) + ' - ' + changFormat(toDate) : ''}</Text>
                                                    </View>
                                                    :
                                        ...
                                        <View style={{ flex: 1 }}>
                                            {
                                                0 < record.length ? <>
                                                    ...
                                                    <View style={[{ marginStart: 10, marginEnd: 10, marginTop: 10, padding: 10 }]}>
                                                        <View>
                                                            <Text style={styles.listTitle}>List of NSF</Text>
